...
x= pd.read_csv(input_file, usecols=[2, 3, 4, 5, 7, 8, 10, 11, 12, 13, 14, 16, 17, 18, 19, 26])
y= pd.read_csv(input_file, usecols=[0])
y= np.ravel(y)

x_test= pd.read_csv(input_file_test, usecols=[2, 3, 4, 5, 7, 8, 10, 11, 12, 13, 14, 16, 17, 18, 19, 26])
...
for i in range(10):
    ...
    if method == 'RF':
        ...
        clf = KNeighborsClassifier(n_neighbors=538, leaf_size=728, weights='distance', algorithm='kd_tree',
                                   metric='manhattan', p=6)

    elif method == 'ANN':
        clf = MLPClassifier(hidden_layer_sizes=(456, 145), activation='logistic', solver='lbfgs', max_iter=631,
                            alpha=0.0008954, early_stopping=True)
        ...
        clf = GaussianNB(var_smoothing=0.0827428)
